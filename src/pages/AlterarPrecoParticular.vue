    ...
    <div class="tudo">
      <div class="pesquisar">
        ...
        <div class="Box-laranja">
          <h1 class="title">{{ dadosPromocao.nome }}</h1>
          <div class="img"><img :src="dadosPromocao.imagem" alt="Imagem do produto" /></div>
        </div>
      </div>
      ...
      <div class="logoEfiltros">
        ...
        <h1 class="title" id="valorAtual">Valor atual:</h1>
        <input class="text-input" type="text" :value="dadosPromocao.preco" disabled />

        <h1 class="title" id="novoValor">Novo valor:</h1>
        <input
          class="text-input"
          type="number"
...
const repor = async () => {
  try {
    if (!novaEntrada.value || isNaN(novaEntrada.value)) {
      alert('Digite um valor válido para a nova entrada.')
      return
    }
    dadosPromocao.precoNovo = novaEntrada.value
    router.push('/posAlterarPreco')
  } catch (error) {
...
onMounted(async () => {
  try {
    const response = await axios.get(`http://localhost:3333/produto/AP`)
