  ...
  <div class="tela">
    ...
    <div class="tudo">
      ...
      <div class="logoEfiltros">
        ...
        <h1 class="title" id="qtdAtual">Quantidade atual:</h1>
        <input class="text-input" type="text" :value="dadosEstoque.quantidade" disabled />

        <h1 class="title" id="entrada">Entrada:</h1>
        <input
          class="text-input"
          type="number"
          v-model="novaEntrada"
...
const repor = async () => {
  try {
    ...
    dadosEstoque.quantidadeNova = dadosEstoque.quantidade + parseInt(novaEntrada.value)
    console.log(dadosEstoque.quantidadeNova)
    router.push('/posReporEstoque')
  } catch (error) {
...
onMounted(async () => {
  try {
    const response = await axios.get(`http://localhost:3333/produto/RE`)
