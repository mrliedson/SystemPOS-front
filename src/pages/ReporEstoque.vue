<template>
  <div class="tela">
    ...
    <!-- Título da tela -->
    <h1 class="title">REPOR ESTOQUE</h1>
          ...
          <div class="item" v-for="produto in produtosFiltrados" :key="produto.codigo_barras">
            <div class="row">
              ...
              <div class="info">
                ...
                <div class="dado">
                  <button class="vencer" @click="Repor(produto)">Repor estoque</button>
                </div>
              </div>
            </div>
          </div>
...
onMounted(async () => {
  try {
    const response = await axios.get(
      `http://localhost:3333/produto/RE/${cadastroFuncionario.value.dadosLogin.codEmpresa}`,
    )
...
const produtosFiltrados = computed(() => {
  ...
  let lista = produtos.value.filter((produto) => {
    return (
...
async function Repor(produto) {
  try {
    reporEstoque.value.dadosEstoque.nome = produto.nome
    reporEstoque.value.dadosEstoque.marca = produto.marca
    reporEstoque.value.dadosEstoque.estoqueMin = produto.estoque_min
    reporEstoque.value.dadosEstoque.estoqueMax = produto.estoque_max
    reporEstoque.value.dadosEstoque.quantidade = produto.quantidade
    reporEstoque.value.dadosEstoque.imagem = produto.imagem
    reporEstoque.value.dadosEstoque.id = produto.id
